        ...
        buttonGetData.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!query.isEmpty()) {
                    ...
                    String url = "https://pixabay.com/api/?key=" + BuildConfig.UNSPLASH_ACCESS_KEY + "&q=" + query.replaceAll(" ", "+") + "&image_type=photo&lang=ru&orientation=vertical&per_page=" + LENGTH;
                    Log.d("Query", url);
                    getPhotos(url);
                }
            }
        ...
        editTextText.setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_UP) {
                    ((InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(editTextText.getWindowToken(), 0);
                    buttonGetData.performClick();
                    return true;
                }
                return false;
            }
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < LENGTH && i < hits.length(); ++i) {
                            photoList.add(new Photo(hits.getJSONObject(i).getString("largeImageURL")));
                        }
